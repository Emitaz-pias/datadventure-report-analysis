    ...
    <main>
            ...
            {chartsPageNo === "1"
              ? sliderPage1.map((chart) => (
                  <div
                    ...
                    onDragStart={(event) => handleDrag(chart, event)}
                    id={`${chart.id}`}
                    draggable={true}
                    className="singleChart  text-center ms-5 "
                    onClick={() => handleSlectChart(chart.id)}
                  >
                    <div>
                      <p>
                        <img
              ...
              : sliderPage2.map((chart) => (
                  <div
                    id={`${chart.id}`}
                    onClick={() => handleSlectChart(chart.id)}
                    onDragStart={() => handleDrag(chart.id)}
                    draggable="true"
                    className="singleChart  text-center ms-5 "
                  >
                    <div>
      ...
      <section className="ms-4 mt-5">
        <div className="ms-5 ps-5 indicatorBox text-center">
          {addOnClicked ? (
            <p
              onDrop={(event) => handleDragOver("dropzone", event)}
              id="dropzone"
              className="tapOn pt-5 mt-3 me-5"
            >
              Place chart here
            </p>
          ) : (
